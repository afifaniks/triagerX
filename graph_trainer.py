...
# %%
all_embeddings = embedding_model.encode(X_df["text"].to_list())
...
for idx, row in X_df.iterrows():
    G.add_node(idx, text=all_embeddings[idx], label=row['owner_id'])
...
for i in tqdm(range(len(X_df))):
    for j in range(i+1, len(X_df)):
        text1 = df.iloc[i]['text']
        text2 = df.iloc[j]['text']

        text1_encode = all_embeddings[i]
        text2_encode = all_embeddings[j]

        similarity = util.cos_sim(text1_encode, text2_encode).squeeze()
...
# %%
# Convert the graph to a format suitable for GNN
adjacency_matrix = nx.to_numpy_array(G)
node_features = [G.nodes[node]['text'] for node in G.nodes()]
...
model = GNNClassifier(input_dim=node_features.shape[1], hidden_dim=128, output_dim=len(torch.unique(labels)))
...
for epoch in range(100):
    optimizer.zero_grad()
    out = model(node_features, torch.nonzero(adjacency_matrix, as_tuple=False).t())
...
model.eval()
with torch.no_grad():
    pred = model(node_features, torch.nonzero(adjacency_matrix, as_tuple=False).t()).argmax(dim=1)
    accuracy = (pred[test_idx] == labels[test_idx]).sum().item() / len(test_idx)
