...
def clean_word_list(item):
    # 1. Remove \r
    current_title = str(item["issue_title"]).replace("\r", " ")
    current_desc = str(item["description"]).replace("\r", " ")
    # 2. Remove URLs
    current_desc = re.sub(
...
def preprocess_csv_dataset(dataset_name):
    print("Preprocessing {0} dataset: Start".format(dataset_name))
    # The CSV file location containing the data for deep learning model training
    open_bugs_json = "./data/{0}/deep_data.csv".format(dataset_name)
    ...
    data = pd.read_csv(open_bugs_json)
    data = data.rename(columns={"assignees": "owner", "issue_body": "description"})
    ...
    for min_train_samples_per_class in [20]:
        ...
        closed_bugs_csv = "./data/{0}/classifier_data_{1}.csv".format(
            dataset_name, min_train_samples_per_class
        )

        data = pd.read_csv(closed_bugs_csv)
        data = data.rename(columns={"assignees": "owner", "issue_body": "description"})
