...
def prepare_dataframe(df: pd.DataFrame, minimum_contribution=300) -> pd.DataFrame:
    ...
    keep_random = np.random.choice(owners, NUM_DEVELOPERS, replace=False)
    df = df[df["owner"].isin(keep_random)]
    logger.info(f"Selected random {len(df['owner'].unique())} developers...")
...
output_file = f"dt_{model_name}_{20}_{NUM_DEVELOPERS}_{sampler_name}"
